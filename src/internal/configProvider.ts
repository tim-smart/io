...
/** @internal */
export const fromMap = (map: Map<string, string>, pathDelim = ".", seqDelim = ","): ConfigProvider.ConfigProvider => {
  const escapedSeqDelim = new RegExp(seqDelim)
  const escapedPathDelim = new RegExp(pathDelim)
  const makePathString = (path: Chunk.Chunk<string>): string => pipe(path, Chunk.join(pathDelim))
  const unmakePathString = (pathString: string): Chunk.Chunk<string> =>
    Chunk.unsafeFromArray(pathString.split(escapedPathDelim))
  const load = <A>(
    path: Chunk.Chunk<string>,
    ...
    const name = pipe(Chunk.last(path), Option.getOrElse(() => "<unnamed>"))
    ...
    return pipe(
      ...
      core.flatMap((value) => parsePrimitive(value, path, name, primitive, escapedSeqDelim))
    ).traced(trace)
  }
  const enumerateChildren = (
    ...
    return core.sync(() => {
      const keyPaths = pipe(Chunk.fromIterable(map.keys()), Chunk.map(unmakePathString))
      return pipe(
        keyPaths,
        Chunk.filter((chunk) => {
          for (let i = 0; i < path.length; i++) {
            const pathComponent = pipe(path, Chunk.unsafeGet(i))
            const element = pipe(chunk, Chunk.get(i))
            if (!(Option.isSome(element) && pathComponent === element.value)) {
              return false
            }
          }
          return true
        }),
        Chunk.map((chunk) => pipe(chunk, Chunk.drop(path.length), Chunk.take(1))),
        Chunk.flatten,
        HashSet.from
      )
    })
...
const extend = <A, B>(
  ...
  const leftPad = Chunk.unfold(
    left.length,
    (index) => index >= right.length ? Option.none : Option.some([leftDef(index), index + 1])
  )
  const rightPad = Chunk.unfold(
    right.length,
    (index) => index >= left.length ? Option.none : Option.some([rightDef(index), index + 1])
  )
...
/** @internal */
const splitPathString = (text: string, escapedDelim: string | RegExp): Chunk.Chunk<string> => {
  return Chunk.unsafeFromArray(text.split(new RegExp(`\\s*${escapedDelim}\\s*`)))
}
...
const parsePrimitive = <A>(
  ...
  return pipe(
    splitPathString(text, escapedDelim),
